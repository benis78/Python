...
# Få det samlede antal brugte rækker i arket
max_row = sheet.UsedRange.Rows.Count

# Find rækker, der skal slettes
rows_to_delete = set()  # Brug et set for at undgå duplikater

for row in range(2, max_row + 1):  # Start fra anden række for at undgå headers
    cell_value_B = str(sheet.Cells(row, 2).Value or "").strip()  # Kolonne B
    cell_value_F = str(sheet.Cells(row, 6).Value or "").strip().lower()  # Kolonne F
    cell_value_A = str(sheet.Cells(row, 1).Value or "").strip()  # Kolonne A
...
# Behandl filnavn og indsæt værdier i specifikke celler
...
# Indlæs filen igen med OpenPyXL for videre behandling
...
# ❌ Stop programmet med en fejlmeddelelse, hvis "QTY" ikke findes
if qty_column is None:
    messagebox.showerror("ERROR", "Kolonnen 'QTY' blev ikke fundet i BOM-arket.")
    os._exit(1)

# ✅ Konverter QTY-kolonnen til numerisk format og håndter fejl
data[qty_column] = pd.to_numeric(data[qty_column], errors='coerce').fillna(0).astype(int)

# ✅ Opdater totalQTY med den korrekte QTY-kolonne
qty = data[qty_column].tolist()
...
# Definer en grå baggrundsfarve (RGB 165,165,165)
header_fill = PatternFill(start_color="A5A5A5", end_color="A5A5A5", fill_type="solid")

# Definer hvid, fed skrift
header_font = Font(color="FFFFFF", bold=True)

# Gør den første række fed, tilføj baggrundsfarve og hvid skrift
for col in range(1, sheet.max_column + 1):  
    cell = sheet.cell(row=1, column=col)
    cell.font = header_font  # Gør teksten hvid og fed
    cell.fill = header_fill  # Tilføj grå baggrund

# Tilføj autofilter til hele første række
sheet.auto_filter.ref = sheet.dimensions  # Anvend filter til hele tabellen

# Sæt alle rækker med indhold (undtagen første række) til 91 pixels i højden
for row in range(2, sheet.max_row + 1):  # Starter fra række 2
    sheet.row_dimensions[row].height = 68.25  # Sæt højden til 91 pixels

# Tilpas kolonnebredden til det bredeste indhold i hver kolonne (undtagen kolonne E)
for col in range(1, sheet.max_column + 1):
    col_letter = get_column_letter(col)  # Konverter kolonneindeks til bogstav
    if col_letter == "E":  # Spring kolonne E over
        continue
    ...
    for row in range(1, sheet.max_row + 1):
        ...
        if cell_value:
            max_length = max(max_length, len(str(cell_value)))  # Find længste værdi

    sheet.column_dimensions[col_letter].width = max_length + 2  # Giv lidt ekstra plads
...
#Pyinstaller --onefile --noconsole ExcelCopyBOM.py
