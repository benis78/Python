...
logging.basicConfig(
    filename=log_file,
    level=logging.DEBUG,  # Ændret fra INFO til DEBUG for mere detaljeret logging
    format='%(asctime)s - %(levelname)s - %(message)s',
    filemode='w'  # Overskriv fil hver gang programmet kører
)
...
class ExcelCopyBOMGUI:
    ...
    def process_bom(self, file_path):
        ...
        try:
            self.update_progress(15, "Applying BOM structure rules...")
            self.handle_bom_structure(sheet)
            logging.info("BOM structure rules applied successfully")
        except Exception as e:
            logging.error(f"Error applying BOM structure rules: {str(e)}", exc_info=True)
            messagebox.showwarning("Warning", "Could not apply all BOM structure rules, but continuing process...")

        # Håndter BOM Structure regler
        try:
            self.update_progress(15, "Cleaning up Part Numbers...")
            
            # Iterer gennem rækkerne i kolonne B og fjern "-X" eller "-bogstav" hvis det findes
            for row in range(2, sheet.UsedRange.Rows.Count + 1):
                part_number = sheet.Cells(row, 2).Value  # Kolonne B
                if part_number is not None:  # Tjek om cellen ikke er tom
                    part_number = str(part_number).strip()  # Konverter til streng og fjern whitespace
                    # Brug regex til at fjerne "-bogstav" i slutningen
                    cleaned_part_number = re.sub(r'-[A-Za-z]$', '', part_number)
                    if cleaned_part_number != part_number:  # Kun log, hvis der er ændringer
                        sheet.Cells(row, 2).Value = cleaned_part_number
                        logging.info(f"Cleaned Part Number in row {row}: {part_number} -> {cleaned_part_number}")

            logging.info("Part Number cleanup completed successfully")
        except Exception as e:
            logging.error(f"Error cleaning up Part Numbers: {str(e)}", exc_info=True)
            messagebox.showwarning("Warning", "Could not clean up Part Numbers, but continuing process...")
        ...
        data['Item'] = [i.rsplit('.0', 1)[0].strip() for i in data['Item']]

        # Generer parentlevels
        parentlevels = [i.rsplit('.', 1)[0].strip() for i in data['Item']]

        # Opret listen over items
        items = [value.strip() for value in data['Item']]
        ...
        for i in parentlevels:
            if i == '':
                ...
                try:
                    ...
                    logging.error(f"Parent item '{i}' not found in items list")
            ...
            if i == '':
                ...
                if ii[e] is not None:
                    try:
                        ...
                        logging.error(f"IndexError: Unable to calculate Total QTY for row {e + 2}. Parent index {ii[e]} is out of range.")
                        totalQTY.append(qty[e])  # Brug kun egen QTY hvis der er en fejl
                else:
                    logging.warning(f"Parent not found for row {e + 2}. Using only own QTY.")
    ...
    def handle_bom_structure(self, sheet):
        ...
        logging.info(f"Starting BOM structure handling. Total rows: {max_row}")
        ...
        for item_number, (start_row, level) in hierarchy.items():
            ...
            if structure_type == "Phantom":
                ...
                logging.info(f"Marked Phantom row {start_row} for deletion")
            ...
            # Håndter Inseparable items
                ...
                for child_item, (child_start, child_level) in hierarchy.items():
                    if child_item.startswith(item_number + '.') and child_level == level + 1:
                        ...
                        logging.info(f"Marked direct child row {child_start} of {item_number} for deletion")
        ...
        logging.info(f"Total rows marked for deletion: {len(rows_to_delete)}")
        if rows_to_delete:
            logging.info(f"Rows to delete: {sorted(rows_to_delete)}")

        # Slet rækker fra bunden og op
        for row in sorted(rows_to_delete, reverse=True):
            try:
                sheet.Rows(row).Delete()
                logging.info(f"Successfully deleted row {row}")
            except Exception as e:
                logging.error(f"Error deleting row {row}: {str(e)}")
