...
def categorize_data(df, file_path):
    """Opdeler data i kategorier i to trin: først normal kategorisering (undtagen piping), derefter piping."""
    ...
    print("\nTilgængelige piping kategorier:")
    for prefix, category in piping_categories.items():
        print(f"  {prefix} -> {category}")
    
    # Step 1: Gruppér items efter deres base nummer
    print("\n=== STEP 1: GRUPPERING AF ITEMS ===")
    item_groups = {}
    for idx, row in df.iterrows():
        item_number = str(row.iloc[0])
        base_number = item_number.split('.')[0]
        if base_number not in item_groups:
            item_groups[base_number] = []
        item_groups[base_number].append((idx, row))  # Gem også index
    
    print(f"Antal grupper fundet: {len(item_groups)}")
    for base_number, group in item_groups.items():
        print(f"Gruppe {base_number}: {len(group)} items")
        # Print første item i gruppen
        _, first_row = group[0]
        print(f"  Første item: {first_row.iloc[0]} -> {first_row.iloc[1]}")
    
    # Step 2: Find piping grupper først
    print("\n=== STEP 2: IDENTIFICERER PIPING GRUPPER ===")
    piping_items = set()  # Gem alle items der tilhører piping grupper
    piping_groups = {}    # Gem piping grupper med deres kategori
    
    for base_number, group in item_groups.items():
        print(f"\nAnalyserer gruppe {base_number}")
        
        # Find parent item (den med base_number)
        parent_row = None
        for idx, row in group:
            if str(row.iloc[0]) == base_number:
                parent_row = row
                break
        
        if parent_row is None:
            print(f"  Advarsel: Ingen parent row fundet for gruppe {base_number}")
            # Brug første række som parent hvis ingen parent findes
            parent_row = group[0][1]
        
        # Tjek om parent part number matcher en piping kategori
        parent_part_number = str(parent_row.iloc[1])
        print(f"  Parent part number: {parent_part_number}")
        
        # Split part number på alle '-' og tjek hver del
        part_groups = parent_part_number.split('-')
        print(f"  Split part number: {part_groups}")
        
        # Tjek hver del af part number for piping prefix
        found_piping = False
        for part in part_groups:
            print(f"  Tjekker del: {part}")
            if part.startswith('BM') or part[:2] in piping_categories:
                ...
                print(f"  MATCH! Gruppe {base_number} tilhører {target_category} (prefix: {prefix})")
                
                # Gem alle gruppens items som piping items
                piping_groups[base_number] = target_category
                for _, row in group:
                    item_number = str(row.iloc[0])
                    part_number = str(row.iloc[1])
                    piping_items.add(item_number)
                    print(f"    Tilføjer {item_number} ({part_number}) til piping gruppe")
                found_piping = True
                break
        
        if not found_piping:
            print(f"  Ingen piping match fundet")
    
    print(f"\nFundet {len(piping_groups)} piping grupper med total {len(piping_items)} items")
    print("Piping grupper:")
    for base_number, category in piping_groups.items():
        print(f"  Gruppe {base_number} -> {category}")
    
    # Step 3: Normal kategorisering (undtagen piping items)
    print("\n=== STEP 3: NORMAL KATEGORISERING ===")
    for _, row in df.iterrows():
        item_number = str(row.iloc[0])
        
        # Skip hvis denne item er del af en piping gruppe
        if item_number in piping_items:
            print(f"Springer over {item_number} da det er en del af en piping gruppe")
            continue
        
        part_number = str(row.iloc[1])
        print(f"\nKategoriserer {item_number} ({part_number})")
        ...
        # Tilføj til den matchende kategori eller Other Items
        target_category = matching_category if matching_category else "Other Items"
        print(f"  Tilføjer til {target_category}")
        
        if target_category not in categorized_data:
            categorized_data[target_category] = pd.DataFrame(columns=df.columns)
        categorized_data[target_category] = pd.concat(
            [categorized_data[target_category], row.to_frame().T],
            ignore_index=True
        )
    
    # Step 4: Tilføj piping grupper til deres kategorier
    print("\n=== STEP 4: TILFØJER PIPING GRUPPER ===")
    for base_number, target_category in piping_groups.items():
        print(f"\nTilføjer piping gruppe {base_number} til {target_category}")
        
        if target_category not in categorized_data:
            print(f"  Opretter ny kategori: {target_category}")
            categorized_data[target_category] = pd.DataFrame(columns=df.columns)
        
        group = [row for _, row in item_groups[base_number]]
        for row in group:
            item_number = str(row.iloc[0])
            part_number = str(row.iloc[1])
            print(f"  Tilføjer {item_number} ({part_number})")
            categorized_data[target_category] = pd.concat(
                [categorized_data[target_category], row.to_frame().T],
                ignore_index=True
            )
    
    # Print endelig kategorisering
    print("\n=== KATEGORISERING AFSLUTTET ===")
    print("\nAlle kategorier med data:")
    for category, data in categorized_data.items():
        if len(data) > 0:  # Kun vis kategorier der har items
            print(f"\n{category}: {len(data)} items")
            print("Items i denne kategori:")
        ...
        for sheet_name, data in categorized_data.items():
            ...
            for idx, row in data.iterrows():
                ...
                if part_number and part_number.strip():
                    ...
                    if found_range:
                        ...
                        print(f"  Kopierer {part_number} fra række {row_num} til {idx+2}")
                        sheet.Range(f"{row_num}:{row_num}").Copy(
                            target_sheet.Range(f"{idx+2}:{idx+2}")
                        )
            ...
            if "Piping" in sheet_name:
                ...
                if last_row > 1:  # Kun hvis der er data
                    print(f"Grupperer rækker 1-{last_row} i {sheet_name}")
...
def process_excel_in_order(file_path, progress_window):
    ...
    try:
        ...
        progress_window.update_progress(10, "Opretter kopi af Excel fil...")
        ...
        progress_window.update_progress(20, "Identificerer kolonner...")
        headers = {}
        for i in range(1, sheet.UsedRange.Columns.Count + 1):
            header = str(sheet.Cells(1, i).Value).strip().upper()
        ...
        progress_window.update_progress(30, "Håndterer Inseparable rækker...")
        rows_to_delete = []
        for row in range(2, sheet.UsedRange.Rows.Count + 1):
        ...
        progress_window.update_progress(40, "Håndterer revisioner...")
        # Scan efter filer først så vi kan tjekke dem
        ...
        # 5. Beregn Total QTY
        progress_window.update_progress(50, "Beregner Total QTY...")
        ...
        progress_window.update_progress(60, "Opretter kategori faner...")
...
def main():
    ...
    try:
        ...
        try:
            ...
            sheet, drawings_col, excel_copy_path = process_excel_in_order(file_path, progress_window)
            
            # Scan efter filer
            progress_window.update_progress(70, "Scanner efter PDF og DWG filer...")
            files = scan_directory_concurrent(network_path)
