...
# Vælg den nye placering for PDF-filerne
new_location = filedialog.askdirectory(title='Choose destination folder to save PDF files')

if new_location == '':
    messagebox.showwarning("Warning", "No destination folder selected. Exiting...")
    os._exit(1)

# Kopier alle PDF-filer til den nye placering med bevarelse af mappestrukturen
for root, dirs, files in os.walk(destPath):
    for file in files:
        if file.endswith(".pdf"):
            file_path = os.path.join(root, file)
            # Konstruer den nye placering for filen med bevarelse af undermapper
            new_file_location = os.path.join(new_location, os.path.relpath(file_path, destPath))
            try:
                # Opret alle undermapper, hvis de ikke allerede findes
                os.makedirs(os.path.dirname(new_file_location), exist_ok=True)
                shutil.copy2(file_path, new_file_location)
            except Exception as e:
                messagebox.showwarning("Warning", f"Error copying file {file}: {e}")

messagebox.showinfo("Success", f"All PDF files copied to {new_location}")
